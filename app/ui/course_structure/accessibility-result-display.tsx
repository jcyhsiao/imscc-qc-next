import { AccessibilityResultType, Resource, EnhancedAxeResult } from "@/app/lib/definitions";
import { Badge, Grid, Well, Accordion, CheckboxGroup, Checkbox, Flex, Disclosure, Switch, DisclosureTitle, DisclosurePanel, Link, Text, View } from "@adobe/react-spectrum";
import { useState } from "react";
import { capitalize, QC_BADGES } from "@/app/ui/helpers";

export function AccessibilityResultsDisplay({ resources }: { resources: Resource[] }) {
  ...
  const allParentResourceTypes = new Set(resources.map(resource => resource.clarifiedType || 'tbd'));
    ...
    <>
        ...
        <CheckboxGroup label="Result Types" name='result type' value={selectedResultTypes} onChange={setSelectedResultTypes}>
          {
            Array.from(allResultTypes).map(type => (
              <Checkbox key={type} value={type}>{type !== 'incomplete' ? capitalize(type) : 'Investigate'} ({allResults.filter(result => result.type == type).length})</Checkbox>
            ))
          }
        </CheckboxGroup>
        <CheckboxGroup label="Found in Parent Resource" name='parent resource type' value={selectedResourceTypes} onChange={setSelectedResourceTypes}>
          {
            Array.from(allParentResourceTypes).map(type => (
              <Checkbox key={type} value={type}>{capitalize(type)} ({allResults.filter(result => result.parentItemType === type).length})</Checkbox>
            ))
          }
        </CheckboxGroup>
      ...
      <Accordion>
        {resources.sort((a, b) => a.title.localeCompare(b.title)).map(resource => {
          const resourceResults = allResults.filter(result => result.parentItemIdentifier === resource.identifier);

          const filteredResultsCount =
          resourceResults.filter(result => selectedResultTypes.includes(result.type)).length;
...
export function AccessibilityResultDisplay({ type, result, selectedResultTypes, selectedParentResourceTypes }: AccessibilityResultDisplayProps) {
  ...
  const impactBadge = result.impact ? QC_BADGES.accessibilityViolationImpact[result.impact] : null;
  const typeBadge = result.type ? QC_BADGES.accessibilityResultType[result.type as AccessibilityResultType] : null;

  return (
    <View borderWidth='thin' borderColor='dark' borderRadius='medium' padding='size-200' marginBottom='size-100' isHidden={isHidden}>
          ...
          <Text>{result.description}</Text><br />
          {nodeHtml !== '' &&
            <>
          ...
          <Text>CSS Selector:</Text><br />
          <Text>{nodeTargets}</Text><br />
